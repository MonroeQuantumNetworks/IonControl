...
class AFG3101(RFSource):
    ...
    def setFreq(sef, freq, **kwargs):
        cmd = 'sour:freq {0}'.format(str(freq))
        self.inst.write(cmd)

    def getAmp(self, **kwargs):
        cmd = 'sour:volt:imm:ampl?'
        amp = self.inst.ask(cmd)
        return float(amp)

    def setAmp(self, amp, **kwargs):
        cmd = 'sour:volt:ampl {0}'.format(str(amp))
        self.inst.write(cmd)

    def getPhase(self, **kwargs):
        raise AFG3101Error(code=2)
