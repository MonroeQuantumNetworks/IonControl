...
SequenceLengths = range(2,201,20)
Randomizations = 50   # at length 100
filename = "Randomized-Clifford-2014-11-27.xml"
path = ""
...
def createRandomization(length):
    ...
    """
    state = i
    randomization = [ StringList[random.randrange(0,len(StringList))] for _ in range(length) ]
    for g in randomization:
        state = state * StringToClifford[g]
    inversionstrings = CliffordToString[state.inv()]
    randomization.append( inversionstrings[random.randrange(0,len(inversionstrings))] )
...
def randomizations(length):
    return 25 + int(Randomizations * (length/100.)**2)
...
if __name__=="__main__":    
    ...
    for length in SequenceLengths:
        for randindex in range(randomizations(length)): 
            sequence = createRandomization(length)
            gateSequence( root, sequence, "{0}".format(index), index, 'd')
            outcomes.append('d')
