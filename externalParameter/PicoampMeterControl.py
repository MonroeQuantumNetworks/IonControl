...
#         self.voltagePlottedTrace_3.trace.filenameCallback =  partial( self.plottedTrace.traceFilename, self.meterState.filename )           
    ...
    def takeLoggingPoint(self):
        ...
        self.trace.voltage = numpy.append( self.trace.voltage, self.meterState.voltage.toval('V') )
#         self.trace.voltage_2 = numpy.append( self.trace.voltage_2, self.meterState.voltage_2.toval('V') )
#         self.trace.voltage_3 = numpy.append( self.trace.voltage_3, self.meterState.voltage_3.toval('V') )
...
#             self.meter_3.voltageEnable(True)
    ...
    def initPoint(self):
        if self.currentIndex<len(self.scanList) and not self.stopRequested:
            ...
            QtCore.QTimer.singleShot(self.meterState.settlingDelay.toval('ms'), self.takeScanPoint )
        else:
