...
##        plt.draw()
    ...
    def update_scan_type(self):
        ...
        if (self.scan_entry.currentText()=='Continuous'):
            ...
            self.scan_range_low_sb.setRange(-30,30)
            self.scan_range_high_sb.setDisabled(False)
            self.scan_range_high_sb.setRange(-30,30)
...
##            plot_thread = UpdatePlotThread(self)
##            self.threads.append(plot_thread)
            exp_thread = ExpThread(self, scan_vals, self)
            self.threads.append(exp_thread)
            exp_thread.start()
##            plot_thread.start()
        else:
