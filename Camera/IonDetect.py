...
class IonROI:
    def __init__(self, center: np.ndarray, brightPoints: np.ndarray = None, radius = 1):
        ...
        self.leftBound = np.round(self.center[0] - boundRadius)
        self.rightBound = np.round(self.center[0] + boundRadius)
        self.upBound = np.round(self.center[1] - boundRadius)
        self.lowBound = np.round(self.center[1] + boundRadius)
...
class IonDetection:
    ...
    def idIons(self):
        ...
        for i in sort_brtPnts:
            ...
            y /= len(i)
            cent.append([[x, y], i])
        sorted(cent, key = lambda point: point[0][1])

        for i in range(self.numIons):
            self.IonDict[self.formatKey(i)] = IonROI((cent[i][0][0], cent[i][0][1]), radius = 2)
